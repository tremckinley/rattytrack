...
export async function GET() {
    try {
        ...
        const { data: meetings, error: meetingsError } = await supabase
            .from("meetings")
            .select("id, title, scheduled_start, video_id")
        ...
        // 3. Filter meetings that don't have completed transcripts
        const pendingMeetings = meetings.map((m) => ({
            id: m.id,
            title: m.title,
            date: m.scheduled_start,
            videoId: m.video_id,
            status: m.video_id && completedVideoIds.has(m.video_id) ? "completed" :
                m.video_id && processingVideoIds.has(m.video_id) ? "transcribing" :
                    "idle"
